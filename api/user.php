    ...
    case "GET":
        ...
        if (isset($path[4]) && is_numeric($path[4])) {
            ...
            while ($userrow = mysqli_fetch_array($getuserrow)) {
                $json_array['rowUserdata'] = array('id' => $userrow['userid'], 'username' => $userrow['username'], 'email' => $userrow['usermail'], 'status' => $userrow['status'],);
            }
            ...
            if (mysqli_num_rows($alluser) > 0) {
                while ($row = mysqli_fetch_array($alluser)) {
                    $json_array["userdata"][] = array("id" => $row['userid'], "username" => $row["username"], "email" => $row["usermail"], "status" => $row["status"]);
                }
